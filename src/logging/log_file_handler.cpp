...
void LogDirectoryManager::update_current_file() {
    ...
    } else if (std::filesystem::file_size(log_files.back().second) / (1024.0 * 1024.0) > _max_file_size) {
        new_file_path = _directory_path / (_file_prefix + "-" + std::to_string(log_files.back().first + 1) + ".log");
    } else {
        new_file_path = _directory_path / (_file_prefix + "-" + std::to_string(log_files.back().first) + ".log");
    }
    if (!new_file_path)
        return;
...
void LogDirectoryManager::fetch_directory_log_files(std::vector<std::pair<int, std::filesystem::path>>& result) const {
    ...
    std::sort(result.begin(), result.end(),
              [](std::pair<int, std::filesystem::path>& a, std::pair<int, std::filesystem::path>& b) -> bool {
